...
async function fetchCommunityCommentsPage(postId: string, from: number, to: number) {
  ...
  const attempts = [
    'id, post_id, nickname, author_user_id, password_hash, ip_prefix, content, created_at',
    'id, post_id, nickname, author_user_id, ip_prefix, content, created_at',
    'id, post_id, nickname, author_user_id, content, created_at',
    'id, post_id, nickname, password_hash, content, created_at',
    'id, post_id, nickname, content, created_at',
  ]
...
export async function POST(request: NextRequest) {
  try {
    ...
    let response: { data: CommentRow | null; error: unknown } | null = null
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!user) {
      return Response.json({ message: '로그인한 사용자만 댓글을 삭제할 수 있습니다.' }, { status: 401 })
    }
    ...
    if (commentError || !comment) {
      return Response.json({ message: '댓글을 찾을 수 없습니다.' }, { status: 404 })
    }
